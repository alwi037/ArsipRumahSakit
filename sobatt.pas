...
begin
  if asalscan = 0 then
  begin
    namapdf := Obat.Edt1.Text+'-'+obat.Edt2.Text+'-'+waktu+'Surat-masuk.pdf';
    PDFku:= TfrxCustomExportFilter(frxdbdtst2);
    PDFku.ShowDialog :=False;
    PDFku.FileName := ExtractFilePath(Application.ExeName)+'\FileObatPDF\'+namapdf;
    frxrprt1.PrepareReport(False);
    frxrprt1.export(PDFku);
    obat.lbl8.caption :=namapdf;
  end;
  PObat.VirtualTable1.clear;
