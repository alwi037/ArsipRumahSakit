...
procedure TFScanDok.Button1Click(Sender: TObject);
begin
if VirtualTable1.IsEmpty then
ShowMessage('Data Masih Kosong') else
Close;
FSimpanScanDokter.showmodal;
end;

procedure TFScanDok.Button2Click(Sender: TObject);
var
  sourceIndex : Integer;
  source : TTwainSource;
begin
...
procedure TFScanDok.Button3Click(Sender: TObject);
begin
if VirtualTable1.IsEmpty then
...
procedure TFScanDok.DelphiTwain1TwainAcquire(Sender: TObject;
  const Index: Integer; Image: TBitmap; var Cancel: Boolean);
...
function TFScanDok.NamaGambar: string;
begin
